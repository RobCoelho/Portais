...
def mariadb_selecionar_imovel():
    try:
        mariadb_connection = mariadb.connect(host='auroratest-cluster.cluster-cp5hgxylykwl.us-east-1.rds.amazonaws.com',
    ...
    try:
        cursor.execute("SELECT DISTINCT mut.NOME FROM CDIMCMUT mut JOIN CDSITE2 cd2 "
                       "WHERE mut.DESCRICAO = cd2.CAPTION AND mut.TAG_MESTRE<>''")
    ...
    try:
        cursor.execute("SELECT DISTINCT mut.TAG_MESTRE FROM CDIMCMUT mut JOIN CDSITE2 cd2 "
                       "WHERE mut.DESCRICAO = cd2.CAPTION AND mut.TAG_MESTRE<>''")
    ...
    try:
        ...
        #for linha in img:
            #print(str(linha))
            #print('Até aqui rodou')
            #item = (str(linha))
            #cod = str(item[0])
            #print(cod)
            #path = str(item[1])
            #print(path)
            #imagens.append(str(item))
    ...
    def lista_tags():
        ...
        while y < len(result):
            ...
            while z < len(result1):
                ...
                z = (z + 1)
            fotos = root.createElement('Fotos')
            imoveis.appendChild(fotos)
            foto = root.createElement('Foto')
            ...
            for linha in img:
                ...
                nomes.append(path)
                if codigo == cod:
                    arquivo = root.createElement('NomeArquivo')
                    arquivo.appendChild(root.createTextNode(path))
                    foto.appendChild(arquivo)
                    urlFoto = (urlbase + codigo + '/' + path)
                    urlArquivo = root.createElement('URLArquivo')
                    urlArquivo.appendChild(root.createTextNode(urlFoto))
                    foto.appendChild(urlArquivo)
                    fotoPrincipal = root.createElement('Principal')
                    foto.appendChild(fotoPrincipal)
                    print(str(codigo) + ' / ' + str(cod))
            y = (y + 1)
        print(str(codigos))
